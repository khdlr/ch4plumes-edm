# Taken from https://github.com/yiyixuxu/denoising-diffusion-flax/blob/main/denoising_diffusion_flax/sampling.py
...
def model_predict(
  state, x, x0, t, ddpm_params, self_condition, is_pred_x0, use_ema=True
):
  if use_ema:
...
def ddpm_sample_step(
  state, rng, x, t, x0_last, ddpm_params, self_condition=False, is_pred_x0=False
):
  ...
  if self_condition:
    ...
    x0, v = model_predict(
      state, x, None, batched_t, ddpm_params, self_condition, is_pred_x0, use_ema=True
    )

  # make sure x0 between [-1,1]
  x0 = jnp.clip(x0, -1.0, 1.0)

  posterior_mean, posterior_log_variance = get_posterior_mean_variance(
    x, t, x0, v, ddpm_params
  )
